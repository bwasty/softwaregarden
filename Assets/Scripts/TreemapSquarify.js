...
class Container {
    ...
    function getCoordinates(row : float[]) {
...
class Treemap {
	...
	function generate() {
        ...
        var treemapMultidimensional = function(data : Array, width, height, xoffset, yoffset) {
            xoffset = (typeof xoffset === "undefined") ? 0 : xoffset;
            yoffset = (typeof yoffset === "undefined") ? 0 : yoffset;
        ...
        //                  boxes in this row
        var calculateRatio = function(row : float[], length) {
            var min = Mathf.Min(row);
            var max = Mathf.Max(row);
        ...
        var improvesRatio = function(currentrow : float[], nextnode, length) {
            var newrow; 
